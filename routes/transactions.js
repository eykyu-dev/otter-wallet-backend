 ...
 */
router.post("/sync", verifyTokenAndExtractUserId, async (req, res, next) => {
  try {
    const user_id = req.user_id;
    const items = await db.getItemIdsForUser(user_id);
    console.log(items);
 ...
 *
 */
router.get("/list", verifyTokenAndExtractUserId, async (req, res, next) => {
  try {
    const user_id = req.user_id;
    const maxCount = req.params.maxCount ?? 50;
    const transactions = await db.getTransactionsForUser(user_id, maxCount);
